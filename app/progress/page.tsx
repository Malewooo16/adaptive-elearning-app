...
const UserProgressDashboarde = async () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-gray-50 min-h-screen">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border-l-4 border-blue-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-lg font-bold">{insights.promotion}</p>
            </div>
          </div>
                ...
                <div className="space-y-2 text-sm">
                  {insights.difficultyInsights.map((diff) => (
                    <div key={diff.difficulty} className="flex justify-between">
                      ...
                      <span>
                        {Math.round(diff.accuracy * 100)}% ({diff.count}{" "}
                        questions)
                      </span>
                    </div>
                  ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-500">Stages Completed</p>
              <p className="text-xl font-semibold">
                {knowledgeStates.filter((k) => k.hasLearned).length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">
                {Math.round((insights?.accuracy ?? overallAccuracy) * 100)}%
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">
                {formatTime(
                  knowledgeStates.reduce((acc, k) => acc + k.totalTimeSpent, 0)
                )}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">
                {Math.round(avgSkillMastery * 100)}%
              </p>
            </div>
          </div>
        </div>
            ...
            {knowledgeStates.map((stage) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-3">
                  <div>
                    Correct: {stage.correctAnswers}/{stage.totalQuestions}
                  </div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all duration-300 ${
                      stage.hasLearned ? "bg-green-600" : "bg-yellow-600"
                    }`}
                    style={{width: `${stage.mastery * 100}%`}}
                  ></div>
                </div>

                {stage.assessmentNumber > 0 && (
                  <p className="text-xs text-gray-500 mt-2">
                    Assessments completed: {stage.assessmentNumber}
                  </p>
                )}
              </div>
            ))}
      ...
      {/* AI Skill Insights */}
      {insights && insights.skillMastery && (
        <div className="bg-white rounded-lg shadow-sm p-6 mt-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-purple-600" />
            AI-Analyzed Skill Performance
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {insights.skillMastery.map((skillData, index) => (
              <div key={index} className="border rounded-lg p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-blue-600">
                    {Math.round(skillData.avgPosteriorP_L * 100)}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                  <div
                    className="bg-purple-600 h-2 rounded-full transition-all duration-300"
                    style={{width: `${skillData.avgPosteriorP_L * 100}%`}}
                  ></div>
                </div>
              </div>
            ))}
